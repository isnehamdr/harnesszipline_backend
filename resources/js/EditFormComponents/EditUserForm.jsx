...
//     // Populate form with user data when editingUser changes
//     useEffect(() => {
//         if (editingUser) {
//             setUserForm({
...
//     // Validate passwords (only if they are being changed)
...
//         // Append all form data that have values
...
const EditUserForm = ({
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(userForm).forEach(key => {
            // Skip email — it's display-only and shouldn't be sent
            if (key === "email") return;
            if (userForm[key] !== null && userForm[key] !== "") {
                formData.append(key, userForm[key]);
            }
