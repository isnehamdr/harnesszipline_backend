...
const EditServiceForm = ({
    ...
    useEffect(() => {
        if (editingService) {
            ...
            let metaDataString = "";
            if (editingService.meta_data) {
                if (typeof editingService.meta_data === 'object') {
                    // If it's already an object, stringify it with pretty print
                    metaDataString = JSON.stringify(editingService.meta_data, null, 2);
                } else {
                    // If it's a string, try to parse and then stringify to ensure format
                    try {
                        const parsed = JSON.parse(editingService.meta_data);
                        metaDataString = JSON.stringify(parsed, null, 2);
                    } catch {
                        // If parsing fails, use as is
                        metaDataString = editingService.meta_data;
                    }
                }
            }
    ...
    const handleSubmit = async (e) => {
        ...
        if (serviceForm.meta_data && serviceForm.meta_data.trim() !== "") {
            try {
                ...
                const parsedMeta = JSON.parse(serviceForm.meta_data);
            ...
            // If meta_data is empty, send null or empty object based on your backend requirements
            formData.append("meta_data", JSON.stringify(null));
        }
