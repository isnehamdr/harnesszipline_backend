...
const AddServiceForm = ({ setShowForm, setReloadTrigger }) => {
    const [submitting, setSubmitting]         = useState(false);
    const [validationErrors, setValidationErrors] = useState({});
    const [imagePreview, setImagePreview]     = useState(null);
    const [imageFile, setImageFile]           = useState(null);
    ...
    const validateJSON = (str) => {
        if (!str || str.trim() === "") { setJsonError(""); return true; }
        try {
            const parsed = JSON.parse(str);
            // Reject bare "null" — we want an actual object/array
            if (parsed === null) { setJsonError("JSON must be an object or array, not null"); return false; }
            setJsonError("");
            return true;
        } catch {
            setJsonError("Invalid JSON format");
    ...
    const resetForm = () => {
        setServiceForm({ name: "", short_description: "", long_description: "", is_featured: false, is_archived: false, meta_data: "" });
        setImagePreview(null);
        setImageFile(null);
        setJsonError("");
        setValidationErrors({});
    ...
    const handleChange = (e) => {
        const { name, value, type, checked } = e.target;
        setServiceForm(prev => ({ ...prev, [name]: type === "checkbox" ? checked : value }));
        if (validationErrors[name]) {
            setValidationErrors(prev => { const n = { ...prev }; delete n[name]; return n; });
        }
    };

    const handleQuillChange  = (content) => setServiceForm(prev => ({ ...prev, long_description: content }));

    const handleMetaDataChange = (value) => {
        setServiceForm(prev => ({ ...prev, meta_data: value }));
        validateJSON(value);
        if (validationErrors.meta_data) {
            setValidationErrors(prev => { const n = { ...prev }; delete n.meta_data; return n; });
        }
    };
    ...
    const handleImageChange = (e) => {
        ...
        if (file.size > MAX_IMAGE_SIZE) { alert(`Image exceeds 2 MB. Current: ${formatFileSize(file.size)}`); return; }
        setImageFile(file);
        const reader = new FileReader();
    ...
    const toggleFeatured = () => setServiceForm(prev => ({ ...prev, is_featured: !prev.is_featured }));
    const toggleArchived  = () => setServiceForm(prev => ({ ...prev, is_archived:  !prev.is_archived  }));
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateJSON(serviceForm.meta_data)) return;
        ...
        // meta_data: only send if the field has a valid non-empty JSON value
        const metaTrimmed = serviceForm.meta_data.trim();
        if (metaTrimmed !== "") {
            // Already validated above — safe to append as-is (it's a JSON string)
            formData.append("meta_data", metaTrimmed);
        }

        // Booleans as "1"/"0" strings — controller checks for === '1'
        formData.append("is_featured", serviceForm.is_featured ? "1" : "0");
        formData.append("is_archived", serviceForm.is_archived ? "1" : "0");
                ...
                {Object.keys(validationErrors).length > 0 && (
                    <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
                        ...
                        <ul className="mt-1 text-xs text-red-600 list-disc list-inside">
                            {Object.entries(validationErrors).map(([field, errors]) => (
                                <li key={field}>
                                    {Array.isArray(errors) ? errors[0] : typeof errors === "string" ? errors : "Invalid value"}
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={submitting}
                            placeholder="Enter service name"
                            className={`w-full px-3 py-2 border ${validationErrors.name ? "border-red-500" : "border-gray-300"} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent`}
                        />
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            disabled={submitting}
                            placeholder="Enter short description"
                            className={`w-full px-3 py-2 border ${validationErrors.short_description ? "border-red-500" : "border-gray-300"} rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none`}
                        />
                        {validationErrors.short_description && <p className="mt-1 text-xs text-red-500">{validationErrors.short_description[0]}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        {validationErrors.long_description && <p className="mt-1 text-xs text-red-500">{validationErrors.long_description[0]}</p>}
                        <style>{`
                            .quill-wrapper .ql-container { border: none; min-height: 150px; font-size: 0.875rem; }
                            .quill-wrapper .ql-toolbar { border: none; border-bottom: 1px solid #e5e7eb; background: #f9fafb; }
                            .quill-wrapper .ql-container.ql-snow { border: none; }
                            .quill-error .ql-container, .quill-error .ql-toolbar { border-color: #ef4444; }
                        `}</style>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-indigo-500 transition-colors relative bg-gray-50">
                            {imagePreview ? (
                                <div className="space-y-2">
                                    <div className="relative group inline-block">
                                        <img src={imagePreview} alt="Preview" className="h-32 w-auto max-w-full object-cover rounded-lg shadow mx-auto" />
                                        <button type="button" onClick={removeImage} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <X size={12} />
                                        </button>
                                    </div>
                                    <p className="text-xs text-gray-600">{imageFile?.name} ({formatFileSize(imageFile?.size)})</p>
                                </div>
                            ) : (
                                <div className="space-y-2">
                                    <Upload className="mx-auto h-8 w-8 text-gray-400" />
                                    <p className="text-sm text-gray-600">Click to upload service image</p>
                                    <p className="text-xs text-gray-500">Max 2 MB · JPEG, PNG, WEBP</p>
                                </div>
                            )}
                            <input
                                type="file"
                    ...
                    <div>
                        ...
                        <div className={`border rounded-lg overflow-hidden ${jsonError ? "border-red-500" : "border-gray-300"}`}>
                            <AceEditor
                                mode="json"
                                theme="github"
                                onChange={handleMetaDataChange}
                                value={serviceForm.meta_data}
                                name="meta_data_editor_add"
                                editorProps={{ $blockScrolling: true }}
                                setOptions={{ showLineNumbers: true, tabSize: 2, fontSize: 14, showGutter: true, highlightActiveLine: true, useWorker: false }}
                                width="100%"
                                height="180px"
                            />
                        </div>
                        {jsonError && <p className="mt-1 text-sm text-red-600">{jsonError}</p>}
                        {validationErrors.meta_data && <p className="mt-1 text-xs text-red-500">{validationErrors.meta_data[0]}</p>}
                        <p className="mt-1 text-xs text-gray-500">Optional. Enter a valid JSON object, e.g. {`{"key": "value"}`}</p>
                    </div>

                    {/* Toggle Switches */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
                        {[
                            { icon: <Star size={18} className="text-gray-600" />, label: "Featured Service", key: "is_featured", toggle: toggleFeatured },
                            { icon: <Archive size={18} className="text-gray-600" />, label: "Archive Service", key: "is_archived", toggle: toggleArchived },
                        ].map(({ icon, label, key, toggle }) => (
                            <div key={key} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                <div className="flex items-center space-x-3">{icon}<span className="text-sm font-medium text-gray-700">{label}</span></div>
                                <button
                                    type="button"
                                    onClick={toggle}
                                    disabled={submitting}
                                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${serviceForm[key] ? "bg-indigo-600" : "bg-gray-300"}`}
                                >
                                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${serviceForm[key] ? "translate-x-6" : "translate-x-1"}`} />
                                </button>
                            </div>
                        ))}
                    </div>
